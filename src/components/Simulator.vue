<template>
  <template v-if="!isUsingSimulator">
    <button
      class="col-span-full mb-6 bg-emerald-700 rounded-lg p-3 hover:bg-emerald-600"
      @click="isUsingSimulator = true"
    >
      Simulate commons/rares
    </button>
  </template>
  <template v-else>
    <div class="col-span-full mb-6 border border-solid border-container rounded-lg p-3 flex flex-col gap-2">
...
const isUsingSimulator = ref(true);
...
const simulate = () => {
  ...
  let pulls = (props.stats.gems_spent - props.stats.mods.epic * pullCost) / pullCost;
